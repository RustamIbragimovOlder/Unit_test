...
class PaymentFormTest {
    ...
    void paymentTestPay() {
        ...
        CreditCard creditCard = mock(CreditCard.class); // создание заглушки вместо реальной кредитной карты

        PaymentForm paymentForm = new PaymentForm(creditCard); // создание формы оплаты
        paymentForm.pay(1000.0); // передача в оплату 1000 руб

        // тест прохождения оплаты
        verify(creditCard, times(1)).charge(1000.0); // times(1) - метод вызывается один раз
